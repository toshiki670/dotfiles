...
起点は **Ghostty**。OS の外観に応じて背景色と ANSI パレットを切り替える（One Half Light = `#fafafa` / Ayu = `#0b0e14`）。他のツールはその**端末背景（OSC 11）**を検出して追従する。パス表記は `configs/` ソース基準。デプロイ先は `~/` 配下。

## インデックス（ツール別）

| ツール | 設定ファイル（`configs/` 基準） | light テーマ | dark テーマ | 追従方式 | 利用可能テーマの確認 |
| --- | --- | --- | --- | --- | --- |
| **Ghostty** | `configs/ghostty/config`（`theme =` 行） | One Half Light | Ayu | Ghostty ネイティブ。`theme = light:…,dark:…` で OS 外観に追従 | `ghostty +list-themes` |
| **Fish**（構文色） | `configs/fish/conf.d/90-fish-theme.fish` + `configs/fish/themes/OneHalfLight-Ayu.theme` | `[light]` セクション | `[dark]` セクション | `fish_terminal_color_theme`（OSC 11）→ `fish_config theme choose` が変数変化に追従 | `fish_config theme list` |
| **Neovim** | `configs/nvim/lua/plugins/colorscheme.lua` | `onehalflight`（`sonph/onehalf`） | `ayu-dark`（`Shatur/neovim-ayu`） | `&background`（**起動時**に OSC 11 検出）→ `OptionSet` で colorscheme 切替 | `:colorscheme <Tab>` |
| **eza** | `configs/eza/40-eza.fish`（`EZA_COLORS`） | グレー明度（濃いめ） | グレー明度（薄いめ） | 色付き要素は Ghostty パレット追従（ANSI）。グレーのメタデータのみ `fish_terminal_color_theme` で明度切替 | `man eza_colors` |
| **fzf** | `configs/fish/conf.d/05-fzf-env.fish`（`FZF_DEFAULT_OPTS` の `--color`） | ANSI 追従 | ANSI 追従 | `--color` を ANSI 番号（`-1`=端末デフォルト / `0`〜`15`）で指定。Ghostty パレット追従でシェル配線不要 | `man fzf` の COLOR セクション |
| **git-delta** | `configs/git/configs/delta`（`[delta "theme-light/dark"]`）+ `configs/fish/conf.d/50-delta.fish`（`DELTA_FEATURES`） | `OneHalfLight` | `ayu-dark` | `DELTA_FEATURES` を `fish_terminal_color_theme` に追従。git 実行ごとに反映 | `delta --list-syntax-themes` |
| **bat** | `configs/bat/config`（`--theme-light/dark`） | `OneHalfLight` | `ayu-dark` | bat ネイティブ `--theme="auto"`（bat 自身が OSC 11 検出）。シェル配線不要 | `bat --list-themes` |
...
1. **Ghostty** `configs/ghostty/config`
   `theme = light:<新Light>,dark:<新Dark>` を書き換える（テーマ名は `ghostty +list-themes`）。
2. **Fish** `configs/fish/themes/OneHalfLight-Ayu.theme`
...
3. **Neovim** `configs/nvim/lua/plugins/colorscheme.lua`
