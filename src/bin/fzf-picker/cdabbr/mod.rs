...
pub fn run(abbr_path: String) -> ExitCode {
    if abbr_path.is_empty() {
        eprintln!("usage: cdabbr <abbreviated-path>");
        return ExitCode::FAILURE;
    }

    let Some((base_kind, segments)) = parse_abbr_path(&abbr_path) else {
    ...
    if candidates.is_empty() {
        eprintln!("cdabbr: no matching path for '{abbr_path}'");
        return ExitCode::FAILURE;
    }
