...
fn has_upstream() -> bool {
    match Command::new("git").arg("remote").output() {
        Ok(out) => dotfiles::remote_exists(&String::from_utf8_lossy(&out.stdout), REPOSITORY),
        Err(_) => false,
    }
}
...
fn merge() -> i32 {
    ...
    let rc = run(&["fetch", REPOSITORY]);
    if rc != 0 {
        return rc;
    }

    run(&["merge", UPSTREAM_PATH])
}
...
fn initialize(remote_url: &str) -> i32 {
    ...
    let rc = run(&["remote", "add", REPOSITORY, remote_url]);
    if rc != 0 {
        return rc;
    }

    run(&["fetch", REPOSITORY])
}
