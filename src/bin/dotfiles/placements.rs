...
//! 見落とす。衝突は配線の誤りであって環境依存の事象ではないため、宣言ベースが正しい（`expected` が
//! `home` を受け取るのは `~` 展開のためだけで、gate の判定には使わない）。
//!
...
//!
//! 比較は解決済みパスの完全一致だけを見る。ツリーが `~/.config/foo/` 配下へ展開する一方で別ユニットが
//! `~/.config/foo` というファイルへ output する「ディレクトリ×ファイルの前置衝突」はパスが一致しない
//! ため対象外 ― これも apply 時に失敗する配線ミスだが、#593 が定義する衝突の範囲外として意図的に外す。
//!
