...
//!
//! [`crate::doctor`] のユニット間 output 衝突検出（#593）が使う。
//!
//! #521（prune）が必要とする「今回の期待配置集合」は、本モジュールの [`expected`] へ
//! [`crate::apply::gate::unit_skip_reason`] を通した部分集合（＝現在 gate を満たすユニットだけの
//! 配置先）として呼び出し側が組み立てる。除去候補は「前回 apply 時点のその集合 ∖ 今回のその
//! 集合」（gate 評価後の diff）で、`when` のどのキー（`deps` / `os` / `profile`）が不成立の
//! 原因かは区別しない ― profile の再分類もツール未インストールによる `deps` 不成立も、
//! 「以前は置いていたが今回は置かない」という点で同じ取り残し経路であり、扱いを分ける理由が
//! 無いため（安全性は削除の既定を dry-run・実削除を opt-in にすることで担保する側に置き、
//! 導出側では分岐させない）。前回分の保持は永続台帳（path → owner の登録簿）ではなく、
//! 単純なパス一覧のスナップショットで足りる ― 両辺とも本モジュールの導出結果（宣言由来）
//! なので、user が独自に置いた drop-in ファイルはどちらの集合にも決して現れない。
//!
