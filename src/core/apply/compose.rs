//! ファイル合成経路: dst=ファイルへ条件付き断片を `strategy` で重ねて書き出す。
//!
//! copy のツリー配置（dst=ディレクトリ）と異なり、generate と overlay 明示はここを通る。
//! 評価順の不変条件を実装する: ②overlay は宣言順に `when` 評価し満たす断片だけ採用、
//! ③`preserve = true` の既存 dst は最下層（土台）として断片の下に敷く。合成後の出力には
//! `locals` の `@@name@@` 注入を通す（空マップなら何もせず通過）。
//! 不変条件①（ユニット gate 先・false で短絡）は [`crate::core::apply`] が前段で担う。

use super::gate::GateState;
use super::set_mode;
use super::{gate, generate, strategy};
use crate::core::locals::resolve;
use crate::core::manifest::{Manifest, Overlay, Strategy};
