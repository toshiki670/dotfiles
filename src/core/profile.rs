//! `dotfiles profile [<name>]`（§10）: マシンクラスの状態 gate を設定／表示する。
//!
//! 状態の読み書きは [`crate::core::state`]（`~/.config/dotfiles/profile`）。`when = { profile = … }`
//! （[`crate::core::apply::gate`]）はここで書いた現在値と一致する断片だけを採用する。profile 名は固定
//! 集合にしない（任意文字列・状態一致）― エンジンを特定 profile 名へ結合させない（§15・D9）。
