...
//! [`crate::core::apply::generate`] の `cmd`（manifest のコマンドをデータとして実行）と同じ思想で、
//! 新ツールのフック追加に binary 変更・再コンパイルは要らない（configs と疎結合・スケールする）。
//! どのフックが macOS 専用か等の知識は manifest 側（ghostty の `os = "darwin"` ＋ コマンド本体）が
//! 持ち、エンジンは関知しない。
//!
//! 頻度で実行モデルが分かれる（§13.0）:
//! - **`onchange`**（既定）: onchange gate（[`crate::core::onchange`]）を通す。ユニットのソースハッシュ
//!   ＋コマンド内容が前回適用時と同じならスキップ、変化（初回・**コマンド変更**を含む）なら実行する。
...
//! （chezmoi の `command -v` ガード相当を、ツール名を持たずに汎用化）。`onchange` はさらにハッシュを
//! 保存しないので、後で入れたら再実行される。トップレベル `when`（ユニット gate, `deps` / `os`）が
//! false のユニットは配置ごと skip されるため hooks も走らない（＝ `when.os` でフックを分岐できる）。
...
/// （[`crate::core::apply`] が配置後に呼ぶ）。onchange のソースハッシュは 1 回だけ計算して使い回す
