...
// 決定的に与えて apply する。json-shallow overlay（base ＋ rtk(when.dep) ＋ preserve）が
// 実ファイルで「ローカル温存・共有上書き・rtk 条件付き hook」を満たすことを検証する
// （overlay/strategy/when の純機構は上の hermetic 群が網羅。ここは実 config の結線確認）。
// rtk スタブ／PATH 制御は unix 限定。
...
/// rtk 在: ローカル（model / effortLevel）を温存しつつ共有（language）を上書きし、
/// when.dep=rtk を満たして rtk hook が入る。preserve 外のローカルキーは持ち込まれない。
#[cfg(unix)]
#[test]
fn apply_real_claude_settings_preserves_local_and_overrides_with_rtk() {
    ...
    assert!(
        !out.contains("localOnly"),
        "preserve 外のローカルキーは温存されないべき:\n{out}"
    );
    assert!(
