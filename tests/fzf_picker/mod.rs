//! `fzf-picker` の各サブコマンドの **E2E テスト**（assert_cmd で実バイナリを起動して検証）。
//!
//! 内部の純ロジック（パース・行成形・展開）のユニットテストは `fzf-picker` bin の各
//! モジュールの `#[cfg(test)]` 側にある。ここは「ビルドしたバイナリの振る舞い」だけを扱う。
//!
//! # 検証内容（サブコマンド別）
//!
...
//! 制御する。共有ヘルパー（下記）は各サブコマンドのテストファイルから `crate::` で使う。
