...
2. **軸2: 「全リポジトリ」を選んだ場合、どの条件でマシンへ配布するかを決める。**
   ...
   - **特定の profile を選んだマシンだけ** → `configs/yt/manifest.toml` と同じパターンで、`configs/claude/` とは別の独立ユニットを作り、unit-level `when = { profile = "..." }` を付け、同じ `output = "~/.claude"` へ向ける（既存 `configs/claude/` ユニットに `when` を足すと、そのユニットの全ファイルが道連れでゲートされる。`output = "~/.config/fish/conf.d"` を複数ユニットが分担している前例と同じ要領）。
...
3. **配置を決める前に、書いた内容自体にこの判定を適用する（自己適用チェック）。** 本文を読み返し、軸1の失格基準（特定のリポジトリ名〔例: `toshiki670/dotfiles`〕・そのリポジトリ内にしか存在しないパス・ファイル名〔他ルールのファイル名等〕）のいずれかを具体的に参照していないか確認する。`~/.claude/...` のような一般的なパスの参照や、特定ツール名への言及（軸2で扱う）はこれに該当しない。1つでも該当すれば「全リポジトリ」は失格で、「`toshiki670/dotfiles` リポジトリで作業するときだけ」へ回す。
