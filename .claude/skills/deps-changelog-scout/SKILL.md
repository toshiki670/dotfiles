...
5. Issue 化の対象ごとに、`gh issue list --repo toshiki670/dotfiles --state all --search "<依存名など>"` で同内容の Issue が既に無いか確認する(open/closed 問わず)。既存の Issue が見つかった対象はドラフトしない(次の依存へ)。同じ PR に対する再実行で重複起票しないための確認。
...
8. 承認を得たら `gh issue create --repo toshiki670/dotfiles --label <ラベル> --title ... --body ...` で起票する。ラベルは finding の種類に対応するもの(新機能なら `enhancement`、バグ修正/脆弱性対応なら `bug` 等)を選び、`gh label list --repo toshiki670/dotfiles` で実在を確認してから使う。
