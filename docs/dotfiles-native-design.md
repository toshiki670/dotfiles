...
configs/
  ...
  ghostty/
    manifest.toml          # dst=~/.config/ghostty, theme="source",
                           #   when={deps=["ghostty"],os="darwin"}, hooks=[macos-symlink]
    config
```
...
- **未インストール時の skip**: プログラムが PATH に無ければ skip し**ハッシュを保存しない**（後でインストールすれば次回 apply で再実行）。chezmoi の `command -v` ガードを、ツール名を binary に持たず汎用化したもの。
...
`hooks::exec` は `Command` の `current_dir` を設定しない。よって hook は **`dotfiles apply` 実行時のプロセス CWD** を継承する。現状の hooks は絶対パス／`$HOME`／PATH 解決で書かれており CWD 非依存なので問題にならない。

> **将来拡張**: `hooks = [["./script.sh"]]` のような**相対パス**ユースケースを許可するなら、「何を基準に実行するか（ユニット dir／repo root／CWD）」を別途確定する（§16 未決事項）。CWD 継承では実行ディレクトリにより挙動が変わり再現性が無いため、相対パスを許す設計では基準の明示が要る。
...
- [ ] **相対パス hook**（`hooks = [["./script.sh"]]`）を許可する場合の実行基準（ユニット dir / repo root / CWD）の確定。現状は CWD 継承で、絶対パス/`$HOME`/PATH 解決の hooks のみ＝CWD 非依存（§13.3）
