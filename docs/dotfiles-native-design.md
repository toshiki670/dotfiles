...
| **D9 エンジン/テストはツールのライフサイクルから独立** | `dotfiles` バイナリは汎用エンジン、`configs/` の個々のツール（claude / rtk / bat …）は**いつか消える一時的なデータ**。**今のツールが全て入れ替わってもバイナリとテストは無変更で生存する**。判定基準＝「configs から特定ツールを削除/改名したらテストが壊れるか? 壊れるなら defect」。契約テストは hermetic な架空 fixture、実 configs の検証は data-driven 走査で表す（§14） |
...
- mise `YT_BROWSER` は named value としてここに乗る。よって create-only（§13/§15・#467）は「machine-specific 値」用途では不要になり吸収される（残る用途は §9.3 へ）。
- 旧案の git native include は ① では採らない（doctor が include 解決のスコープ依存に振り回されるため。`--global`/`--file` は `--includes` 既定 OFF で local を辿らず誤判定しうる）。なお §13 の「ファイル合成」で使う `[include]` は**共有断片の組み立て用**で別物（repo 管理ファイルを束ねる話）。ここで不採用なのは*マシンローカル値の注入*に include を使う旧案に限る。
...
| `dotfiles secret set <name> <value>` | locals（マシンローカル値）をストアへ設定（§9）。**コマンド名は仮称** — `secret` は秘匿値以外（email/name 等）も扱うため概念とズレる。`local`/`secret` の最終命名は §15 のサブコマンド名確定で決める（CLI 未実装＝互換性の制約は無い） |
...
| `create_`（初回のみ生成・以後は温存。mise の machine-specific config 1件） | 値（`YT_BROWSER` 等）は **locals（named value）が吸収**（§9）。任意のローカル上書きが要る用途は **② 拡張口**（§9.3・後続 issue）。create-only 属性そのものは未スライス（§15） |
...
## 14. テスト方針：エンジン/テストはツールのライフサイクルから独立（D9）
...
8. **エンジン/テストはツールのライフサイクルから独立**（D9・§14）。判定基準＝「configs から特定ツールを削除/改名したらテストが壊れるなら defect」。契約テストは hermetic な架空 fixture、実 configs の検証は data-driven 走査。原則は設計書に一度だけ明文化し、各所で繰り返さない。
