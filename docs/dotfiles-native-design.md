...
1. **宣言（manifest）**: unit が要る値を名前で宣言する。属性は `locals = ["git.email", "git.name"]`。秘匿値は `sensitive = ["github.token"]` で指定（エコー/ログを抑制）。email/name は秘密ではない（commit に載る）ため非 sensitive。
...
4. **注入（generate）**: 生成時に **named placeholder の置換のみ**（既定構文 `@@git.email@@`）でストア値を埋める。条件分岐・関数を持つ汎用テンプレートは導入しない。形式非依存（git config / toml / 何でも効く）。
...
- mise `YT_BROWSER` は named value としてここに乗る。よって create-only（§13/§14・#467）は「machine-specific 値」用途では不要になり吸収される（残る用途は §9.3 へ）。
...
| `dotfiles secret set <name> <value>` | locals（マシンローカル値）をストアへ設定（§9） |
...
| `create_`（初回のみ生成・以後は温存。mise の machine-specific config 1件） | 値（`YT_BROWSER` 等）は **locals（named value）が吸収**（§9）。任意のローカル上書きが要る用途は **② 拡張口**（§9.3・後続 issue）。create-only 属性そのものは未スライス（§14） |
