...
| `always` | 毎 apply 無条件に実行（gate を通さず状態も読み書きしない） | `run_` | 反映対象が dotfiles 管理外で随時変わり、ソースが変わらなくても毎 apply 反映したい（copy / compose と同じ「常に再実行」の性質） |

- **`always` は冪等性が前提**。gate を通さず毎 apply 無条件に走るため、コマンドは何度実行しても同じ結果になるものに限る（`ln -sf` のような上書き系・再構築系）。
...
- **タイミング軸（配置前 / 後）**: 現状フックは配置後（after）固定。もし配置前（before）が要るユースケースが出ても、これは頻度と混ぜず**別キー**（例 `timing = "before" | "after"`）で表す ― 頻度（いつ再実行するか）とタイミング（配置の前か後か）は独立に選べる別軸で、1 キーに畳むと直交する 2 つが絡み合うため。
...
| `run_` フック（毎 apply 無条件・管理外状態への反映） | `hooks` 属性の `frequency = "always"`（gate を通さず毎 apply 実行・冪等前提。§13.0・#546） |
...
- [x] **hooks の実行頻度**（常時 / onchange / 一度きり）→ **`frequency` 属性で確定**（§13.0・#546）。既存 `hooks` エントリの属性として `onchange`（既定・ソース変化時のみ）/ `always`（毎 apply 無条件・冪等前提）を選ぶ（別リストは作らず、構造の分岐と表示・validation の二重化を避ける）。`once` 頻度（chezmoi `run_once_`）・タイミング軸（配置前 / 後）は該当ユースケースが無いため意図的に見送り（YAGNI・出た時点で再検討。タイミングは頻度と直交する別キーで表す）
