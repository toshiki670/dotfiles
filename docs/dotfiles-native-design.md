...
# hooks（該当のみ、§13）。各エントリは cmd（argv・コマンド列）＋ frequency（実行頻度、省略時
# "onchange" ＝ ソース変化時だけ実行。"always" は毎 apply 無条件実行）。
# [[hooks]]
# cmd = ["bat", "cache", "--build"]
...
`hooks` 属性（§6.2・§7）はユニットが宣言する **hook エントリの配列**で、ユニット配置後（after フェーズ）に**宣言順**で実行する。各エントリは `cmd`（argv・コマンド列）＋ `frequency`（実行頻度）を持つ:

```toml
[[hooks]]
cmd       = ["bat", "cache", "--build"]
# frequency = "onchange"   # 省略時の既定。ソース変化時だけ実行（§13.1）
```

エンジンはツール固有知識を持たず argv を**データ**として実行するだけ（`generate` の `cmd` と同じ思想・D9／§15）。どのフックが macOS 専用か等の知識は manifest 側（ghostty の `when.os` ＋ コマンド本体）が持ち、binary は関知しない（新ツールのフック追加に再コンパイル不要）。

### 13.0 実行頻度 `frequency`（#546）

各 hook エントリの実行可否は `frequency`（`kind`/`strategy` と同じ enum 属性の流儀）が決める:
...
本節は `frequency = "onchange"`（既定）の hook にだけ適用する（`always` は本節の gate を一切通さない・§13.0）。onchange の hook は毎 apply で無条件には走らず、**ユニットのソースが前回適用時から変わったか**で gate する。状態は `~/.config/dotfiles/hooks.toml` に「フックキー → 前回適用時のソースハッシュ」で持つ（秘匿値でないので平文・通常パーミッション。破損時は warn して空＝全フック再実行＝冪等なので致命にしない）。
...
**仕様（確定・実装済み #498）**: 相対パスの hook プログラム（`hooks = [{ cmd = ["./script.sh"] }]` のように `argv[0]` が区切り付き相対パス）は、その**ユニットの `manifest.toml` があるディレクトリ基準**で解決する（フックスクリプトは manifest と同じ `configs/<unit>/` に置く想定）。絶対パス／PATH 解決される bare コマンド名（`bat` 等）はこの基準の影響を受けない。
