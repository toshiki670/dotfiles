...
| `dotfiles secret set <name> <value>` | locals（マシンローカル値）をストアへ設定（§9）。**コマンド名は仮称** — `secret` は秘匿値以外（email/name 等）も扱うため概念とズレる。`local`/`secret` の最終命名は §14 のサブコマンド名確定で決める（CLI 未実装＝互換性の制約は無い） |
