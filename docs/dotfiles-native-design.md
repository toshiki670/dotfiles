...
> ✅ **`theme` は配線済み**（S7 / #461）。`When.theme`（`Theme` = auto/dark/light）が状態ファイル `~/.config/dotfiles/theme` の値と一致した時だけ採用する。状態の供給は ambient な PATH/OS と違いファイル由来なので、[`gate`] は評価時に現在のテーマ状態を引数で受け取る（apply 開始時に 1 回読む）。gate キーは `deps` / `os` / `theme` の 3 つ。
...
> ✅ **`theme`（トップレベル属性／`when.theme` の両方）は配線済み**（S7 / #461）。トップレベル `theme`（`ThemeRole` = source/follower/self）はツールのテーマ対応方式の宣言で、`dotfiles color` は `source` のユニットを切替後に reload 駆動する（§10.2.1）。`when.theme`（`Theme` = auto/dark/light）はテーマ状態 gate。いずれも enum 化済みで不正値は load 時に弾く。
