...
> ⚠️ **`theme` は未配線**（#493 時点）。`When` は `deny_unknown_fields` のため、color スライスが `theme` フィールドを足すまで `when = { theme = … }` は **load 時エラー**になる。現役の gate キーは `deps` / `os` の 2 つ（§10.2.1）。
...
これにより、いまバラバラだった3つの分岐ニーズ ―依存 gate（`when.deps`）・OS 分岐（`when.os`）・テーマ追従（color）― が **`when` という1つの語彙**に畳まれる。
...
| `when` | gate の採用条件。`deps`（配列・AND） / `os`（スカラ） / `theme`。**書く位置でスコープが決まる**: トップレベル＝ユニット全体 gate（false で dst も hooks も skip ＝ all-or-nothing）、`[[overlay]]` 内＝その断片だけの採否 | 任意 | ghostty=`{deps=["ghostty"],os="darwin"}` / rtk hook=`{deps=["rtk"]}` |
...
| `locals` | マシンローカル値（named value）の宣言。`sensitive` で秘匿指定（エコー/ログ抑制）（§9） | 任意 | git `user.email`/`user.name` / mise `YT_BROWSER` |
...
マシン固有の値（git identity・mise の `YT_BROWSER` 等）は、ツール別の特殊解（git native include / mise create-only）を採らず、**単一の汎用機構**で扱う。現状の `env "DOTFILES_GIT_EMAIL/NAME"` 注入は廃止する。
...
3. **取得（apply）**: apply が全 unit の `locals` を集約し、ストアに無い値を解決する。
   ...
   - 明示経路: `dotfiles secret set <name> <value>`。
...
- mise `YT_BROWSER` は named value としてここに乗る。よって create-only（§14/§16・#467）は「machine-specific 値」用途では不要になり吸収される（残る用途は §9.3 へ）。
...
新エンジンは不要で、**既存 overlay の source origin を repo からローカル既知ディレクトリへ広げる**話に畳める見込み。唯一の注意は apply の prune 挙動（ディレクトリ copy が source 外ファイルを消すと drop-in を消す）→ ② 設計で「ローカル断片パスは prune 対象外」と決める。
...
| `dotfiles secret set <name> <value>` | locals（マシンローカル値）をストアへ設定（§9）。**コマンド名は仮称** — `secret` は秘匿値以外（email/name 等）も扱うため概念とズレる。`local`/`secret` の最終命名は §16 のサブコマンド名確定で決める（CLI 未実装＝互換性の制約は無い） |
...
- **値** = ユニットの**ソースハッシュ**（`onchange::hash_dir`）。ユニットのデプロイ対象ファイル（`manifest.toml` を除く）を相対パス＋内容で SHA-256 に畳む。**mtime ではなく内容ハッシュ**を採る（mtime は touch / clone で揺れるため。§16 の旧未決項目「mtime vs ソースハッシュ」はここで確定）。
- **未インストール時の skip**: フックの**実行プログラム（`argv[0]`）**が PATH に無ければ skip し**ハッシュを保存しない**（後でインストールすれば次回 apply で再実行）。chezmoi の `command -v` ガードを、ツール名を binary に持たず汎用化したもの。**判定は `argv[0]` のみ**で、`["sh", "-c", "…"]` のように shell 経由で呼ぶ内側のコマンドは含まれない（内側依存が無ければ shell は非ゼロ終了し apply はエラーになる）。そうした内側依存は `when.deps`（§5.5）でユニットごと gate する。
...
**仕様（確定）**: 相対パスの hook プログラム（`hooks = [["./script.sh"]]` のように `argv[0]` が区切り付き相対パス）は、その**ユニットの `manifest.toml` があるディレクトリ基準**で解決する（フックスクリプトは manifest と同じ `configs/<unit>/` に置く想定）。絶対パス／PATH 解決される bare コマンド名（`bat` 等）はこの基準の影響を受けない。

> **現行実装との差分（#498 で追従）**: `hooks::exec` は現状 `Command` の `current_dir` を設定せず、hook は **`dotfiles apply` 実行時のプロセス CWD** を継承する。現状の hooks は絶対パス／`$HOME`／PATH 解決で書かれており CWD 非依存なので顕在化しないが、上記の確定仕様（manifest dir 基準）とは挙動が異なる。実装の追従は #498 で追跡する。
...
| `create_`（初回のみ生成・以後は温存。mise の machine-specific config 1件） | 値（`YT_BROWSER` 等）は **locals（named value）が吸収**（§9）。任意のローカル上書きが要る用途は **② 拡張口**（§9.3・後続 issue）。create-only 属性そのものは未スライス（§16） |
...
| **テンプレート条件**（`{{ if lookPath … }}` / 部分的な if） | **overlay の `when`**（`deps` / `os` / `theme`）。テンプレートエンジンは持たず宣言的 gate で表現（§5.5） |
| シークレット注入（env） | `locals` 属性＋ named value ストア＋ apply 取得（TTY 対話 / `secret set`）。env 注入と git native include は不採用（§9） |
...
- [ ] 合成戦略の宣言方法（`strategy` 明示 vs dst 拡張子からの推測）、`when.os` の複数許容（OR セマンティクス）、`theme` 状態の供給元（§5.5 留保）
- [ ] `when.theme` を build-time color に使う範囲（どのツールを runtime 追従のまま残すか。§10.2.1）
- [ ] create-only 属性（chezmoi `create_` 相当: dst 既存なら上書きしない）＋ mise の `config.toml` 移行。machine-specific **値**（`YT_BROWSER` 等）は S4 の `locals`（§9）が吸収。任意ローカル上書き用途は ② 拡張口（§9.3）へ。残る create-only 属性自体は**どのスライスにも無い**ため S1 で mise を見送り＝**S9 ブロッカー**。後続 issue で追跡
...
- [x] **相対パス hook**（`hooks = [["./script.sh"]]`）の実行基準 → **ユニットの `manifest.toml` ディレクトリ基準で確定**（§13.3）。現行実装は CWD 継承のため差分があり、実装の追従は #498 で追跡（現状の hooks は絶対パス/`$HOME`/PATH 解決のみ＝CWD 非依存で顕在化せず）
- [ ] color 手動固定時の Ghostty 起点の扱い（§10.3）
- [ ] chezmoi 併用時のフォールバック実挙動（§12.3）
- [ ] `dotfiles` のサブコマンド名最終確定（`apply`・`secret`/`local`（§9・§11）は仮称。`secret` は秘匿値以外も扱うため `local` 系への改名を含めて検討）
