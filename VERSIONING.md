...
1. **初期 SemVer 期**: `1.0.0` → `2.3.0`（2018-2021）
2. **単純バージョン期**: `3.0` → `9.0`（2021）
3. **v プレフィックス期**: `v10.0` → `v18.0`（2021-2024）

### 移行方針

全 52 個のバージョンを時系列順に v0.x.x 形式に変換：

- **MINOR**: 時系列順に連番（v0.1.0 → v0.28.0）
- **PATCH**: 元の PATCH 番号を保持
- **最終結果**: `v18.0` → `v0.28.0`
...
| 旧バージョン | 新バージョン | 理由                    |
| ------------ | ------------ | ----------------------- |
| 1.0.0        | v0.1.0       | 最初のバージョン        |
| 1.1.0        | v0.2.0       | 次の MINOR              |
| 1.1.1        | v0.2.1       | PATCH 保持              |
| 8.0          | v0.18.0      | MINOR は連番            |
| 8.4          | v0.18.4      | PATCH 保持              |
| v18.0        | v0.28.0      | 最新（28 番目の MINOR） |
