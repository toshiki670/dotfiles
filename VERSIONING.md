...
1. **初期SemVer期**: `1.0.0` → `2.3.0`
2. **単純バージョン期**: `3.0` → `9.0`
3. **vプレフィックス期**: `v10.0` → `v18.0`

### 移行マッピング

詳細な移行マッピングは以下の通りです：

| 旧バージョン範囲 | 新バージョン範囲 | 備考 |
|----------------|----------------|------|
| 1.0.0 - 1.7.1  | 0.1.0 - 0.1.21 | 初期開発期を0.1.xにまとめる |
| 2.0.0 - 2.3.0  | 0.2.0 - 0.2.6  | メジャーバージョン2を0.2.xへ |
| 3.0 - 9.0      | 0.3.0 - 0.9.0  | 単純バージョンを0.x.0へ |
| v10.0 - v18.0  | 0.10.0 - 0.18.0| vプレフィックスを削除 |
