...
1. **実行場所**: リポジトリのルートディレクトリから実行する必要があります

   ```bash
   cd ~/dotfiles  # またはリポジトリのクローン先
   ./migrate-to-semver-0x.sh
   ```

2. **実行ブランチ**: 任意のブランチから実行可能
   ...
   注意: タグはリポジトリ全体に対してグローバルですが、バックアップファイルは実行したブランチに作成されます

3. **必須ツール**:

   - **Bash 3.2+** - macOS デフォルトの Bash で動作
   - `git` - バージョン管理
   - `gh` - GitHub CLI（認証済み）
   - `jq` - JSON 処理
   - `gh` - GitHub CLI（認証済み）
...
- ✅ Git リポジトリ内で実行されているか
- ✅ リポジトリのルートディレクトリから実行されているか
- ✅ 現在のブランチ（情報表示のみ）
- ✅ 未コミットの変更（ある場合は警告）
- ✅ 必要なコマンドがインストールされているか
- ✅ GitHub CLI が認証済みか
